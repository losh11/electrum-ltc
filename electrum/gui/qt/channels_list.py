...
class ChannelsList(MyTreeView):
    ...
    def format_fields(self, chan: AbstractChannel) -> Dict['ChannelsList.Columns', str]:
        ...
        for subject in (REMOTE, LOCAL):
            if isinstance(chan, Channel):
                ...
                if bal_other != bal_minus_htlcs_other:
                    label += ' (+' + self.parent.format_amount(bal_other -
                                                               bal_minus_htlcs_other, whitespaces=False) + ')'
            else:
        ...
        node_alias = self.lnworker.get_node_alias(
            chan.node_id) or chan.node_id.hex()
        capacity_str = self.parent.format_amount(
            chan.get_capacity(), whitespaces=True)
        return {
    ...
    def force_close(self, channel_id):
        ...
        def task():
            ...
            return self.network.run_from_another_thread(coro)
        WaitingDialog(self, 'please wait..', task,
                      self.on_channel_closed, self.on_failure)
    ...
    def request_force_close(self, channel_id):
        def task():
            ...
            return self.network.run_from_another_thread(coro)
        WaitingDialog(self, 'please wait..', task,
                      self.on_request_sent, self.on_failure)
    ...
    def freeze_channel_for_sending(self, chan, b):
        if self.lnworker.channel_db or self.lnworker.is_trampoline_peer(chan.node_id):
            ...
            self.main_window.show_warning(
                msg, title=_('Channel is frozen for sending'))
    ...
    def get_rebalance_pair(self):
        ...
        if len(selected) == 2:
            idx1 = selected[0]
            idx2 = selected[1]
            channel_id1 = idx1.sibling(
                idx1.row(), self.Columns.NODE_ALIAS).data(ROLE_CHANNEL_ID)
            channel_id2 = idx2.sibling(
                idx2.row(), self.Columns.NODE_ALIAS).data(ROLE_CHANNEL_ID)
    ...
    def create_menu(self, position):
        ...
        selected = self.selected_in_column(self.Columns.NODE_ALIAS)
        if not selected:
            menu.addAction(_("Import channel backup"),
        ...
        if len(selected) == 2:
            ...
            if chan1 and chan2:
                menu.addAction(
                    _("Rebalance"), lambda: self.parent.rebalance_dialog(chan1, chan2))
                menu.exec_(self.viewport().mapToGlobal(position))
            return
        elif len(selected) > 2:
            return
        ...
        if not item:
            return
        channel_id = idx.sibling(
            idx.row(), self.Columns.NODE_ALIAS).data(ROLE_CHANNEL_ID)
        ...
        if chan:
            ...
            if chan.can_be_deleted():
                menu.addAction(
                    _("Delete"), lambda: self.remove_channel_backup(channel_id))
            menu.exec_(self.viewport().mapToGlobal(position))
            return
        chan = self.lnworker.channels[channel_id]
        ...
        if chan.is_closed():
            ...
            if item:
                ...
                if closing_tx:
                    menu.addAction(_("View closing transaction"),
        ...
        if chan.can_be_deleted():
            menu.addSeparator()
            menu.addAction(
                _("Delete"), lambda: self.remove_channel(channel_id))
        menu.exec_(self.viewport().mapToGlobal(position))
    ...
    def do_update_rows(self, wallet):
        if wallet != self.parent.wallet:
            return
        channels = list(wallet.lnworker.channels.values()
                        ) if wallet.lnworker else []
        backups = list(wallet.lnworker.channel_backups.values()
                       ) if wallet.lnworker else []
        if wallet.lnworker:
            self.update_can_send(wallet.lnworker)
        self.model().clear()
        ...
        for chan in channels + backups:
            ...
            items = [QtGui.QStandardItem(field_map[col])
                     for col in sorted(field_map)]
            self.set_editability(items)
            if self._default_item_bg_brush is None:
                self._default_item_bg_brush = items[self.Columns.NODE_ALIAS].background(
                )
            items[self.Columns.NODE_ALIAS].setData(
                chan.channel_id, ROLE_CHANNEL_ID)
    ...
    def get_toolbar(self):
        ...
        h.addStretch()
        self.rebalance_button = EnterButton(
            _('Rebalance'), lambda x: self.on_rebalance())
        self.rebalance_button.setToolTip(
            "Select two active channels to rebalance.")
        self.rebalance_button.setDisabled(True)
        self.swap_button = EnterButton(
            _('Swap'), lambda x: self.parent.run_swap_dialog())
        ...
        self.new_channel_button = EnterButton(
            _('Open Channel'), self.new_channel_with_warning)
...
class ChannelFeatureIcons:
    ...
    def paint(self, painter: QPainter, rect: QRect) -> None:
        ...
        for feat in self.features:
            icon_rect = QRect(
                cur_x, rect.y(), self.ICON_SIZE.width(), self.ICON_SIZE.height())
            feat.rect = icon_rect
            if rect.contains(icon_rect):  # stay inside parent
                painter.drawPixmap(
                    icon_rect, feat.icon().pixmap(self.ICON_SIZE))
