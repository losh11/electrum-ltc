...
class ChannelDetailsDialog(QtWidgets.QDialog, MessageBoxMixin):
    def make_htlc_item(self, i: UpdateAddHtlc, direction: Direction) -> HTLCItem:
        it = HTLCItem(_('Sent HTLC with ID {}' if Direction.SENT ==
                      direction else 'Received HTLC with ID {}').format(i.htlc_id))
        it.appendRow([HTLCItem(_('Amount')), HTLCItem(
            self.format_msat(i.amount_msat))])
        it.appendRow([HTLCItem(_('CLTV expiry')),
                     HTLCItem(str(i.cltv_expiry))])
        it.appendRow([HTLCItem(_('Payment hash')),
    ...
    def on_htlc_added(self, evtname, chan, htlc, direction):
        ...
        self.folders['inflight'].appendRow(
            self.make_htlc_item(htlc, direction))
    ...
    def update(self):
        self.can_send_label.setText(self.format_msat(
            self.chan.available_to_spend(LOCAL)))
        self.can_receive_label.setText(self.format_msat(
            self.chan.available_to_spend(REMOTE)))
        self.sent_label.setText(self.format_msat(
            self.chan.total_msat(Direction.SENT)))
        self.received_label.setText(self.format_msat(
            self.chan.total_msat(Direction.RECEIVED)))
        self.local_balance_label.setText(
            self.format_msat(self.chan.balance(LOCAL)))
        self.remote_balance_label.setText(
            self.format_msat(self.chan.balance(REMOTE)))
    ...
    def show_tx(self, link_text: str):
        ...
        self.window.show_transaction(
            funding_tx, tx_desc=_('Funding Transaction'))
    ...
    def __init__(self, window: 'ElectrumWindow', chan_id: bytes):
        ...
        self.format_msat = lambda msat: window.format_amount_and_units(
            msat / 1000)
        ...
        form_layout_left.addRow(_('Channel type:'), SelectableLabel(
            chan.storage['channel_type'].name_minimal))
        ...
        form_layout_right.addRow(
            _('Total received so far:'), self.received_label)
