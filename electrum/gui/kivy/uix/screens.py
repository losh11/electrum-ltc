...
class HistoryScreen(CScreen):
    ...
    def get_card(self, tx_item):  # tx_hash, tx_mined_status, value, balance):
        ...
        if is_lightning:
            ...
            icon = f'atlas://{KIVY_GUI_PATH}/theming/atlas/light/' + \
                TX_ICONS[status]
...
class SendScreen(CScreen, Logger):
    ...
    def set_ln_invoice(self, invoice: str):
        try:
            ...
            self.app.show_info(
                _("Invoice is not a valid Lightning invoice: ") + repr(e))
            return
        ...
        if invoice.is_lightning():
            if self.app.wallet.lnworker:
                ...
                msg = _("Pay lightning invoice?") + '\n\n' + _("This will send {}?").format(
                    self.app.format_amount_and_units_with_fiat(amount_sat)) + '\n'
    ...
    def _do_pay_onchain(self, invoice: Invoice) -> None:
        ...
        amount = sum(map(lambda x: x.value, outputs)) if not any(
            parse_max_spend(x.value) for x in outputs) else '!'
        coins = self.app.wallet.get_spendable_coins(None)

        def make_tx(rbf): return self.app.wallet.make_unsigned_transaction(
            coins=coins, outputs=outputs, rbf=rbf)
        def on_pay(tx): return self.app.protected(
            _('Send payment?'), self.send_tx, (tx, invoice))
        d = ConfirmTxDialog(self.app, amount=amount,
                            make_tx=make_tx, on_pay=on_pay)
        d.open()
...
class ReceiveScreen(CScreen):
    ...
    def new_request(self):
        ...
        if amount_sat and amount_sat < self.app.wallet.dust_threshold():
            ...
            self.address = addr
        try:
            key = self.app.wallet.create_request(
                amount_sat, message, expiry, self.address, lightning=self.app.wallet.has_lightning())
        except InvoiceError as e:
            self.app.show_error(
    ...
    def expiration_dialog(self, obj):
        ...
        def callback(c):
            ...
            self.expiration_text = pr_expiration_values[c]
        d = ChoiceDialog(_('Expiration date'),
                         pr_expiration_values, self.expiry(), callback)
        d.open()
