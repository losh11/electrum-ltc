...
class ElectrumGui(BaseElectrumGui):
    ...
    def __init__(self, *, config: 'SimpleConfig', daemon: 'Daemon', plugins: 'Plugins'):
        ...
        self.tab_names = [_("History"), _("Send"), _(
            "Receive"), _("Addresses"), _("Contacts"), _("Banner")]
    ...
    def print_balance(self):
        ...
        for i in range(self.num_tabs):
            self.stdscr.addstr(0, 2 + 2*i + len(''.join(
                self.tab_names[0:i])), ' '+self.tab_names[i]+' ', curses.A_BOLD if self.tab == i else 0)
    ...
    def print_addresses(self):
        ...
        messages = map(lambda addr: fmt % (
            addr, self.wallet.get_label(addr)), self.wallet.get_addresses())
        self.print_list(messages,   fmt % ("Address", "Label"))

    def print_edit_line(self, y, label, text, index, size):
        text += " "*(size - len(text))
        self.stdscr.addstr(y, 2, label)
        self.stdscr.addstr(y, 15, text, curses.A_REVERSE if self.pos %
    ...
    def print_list(self, lst, firstline=None):
        ...
        for i in range(self.maxy-4):
            ...
            m = msg[0:self.maxx - 2]
            m = m.encode(self.encoding)
            self.stdscr.addstr(i+2, 1, m, curses.A_REVERSE if i ==
                               (self.pos % self.maxpos) else 0)
    ...
    def run_contacts_tab(self, c):
        if c == 10 and self.contacts:
            out = self.run_popup(
                'Address', ["Copy", "Pay to", "Edit label", "Delete"]).get('button')
            key = list(self.contacts.keys())[
                self.pos % len(self.contacts.keys())]
            if out == "Pay to":
                self.tab = 1
    ...
    def main(self):
        ...
        try:
            ...
            raise Exception(
                "Error with curses. Is your screen too small?") from e
        finally:
    ...
    def network_dialog(self):
        ...
        srv = 'auto-connect' if auto_connect else str(
            self.network.default_server)
    ...
    def run_dialog(self, title, items, interval=2, buttons=None, y_pos=3):
        ...
        self.w = curses.newwin(5 + len(list(items)) *
                               interval + (2 if buttons else 0), 68, y_pos, 5)
        w = self.w
        out = {}
        while True:
            w.clear()
