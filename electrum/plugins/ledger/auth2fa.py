...
class LedgerAuthDialog(QDialog):
        ...
        def pin_changed(s):
            if len(s) < len(self.idxs):
                ...
                if not constants.net.TESTNET:
                    ...
                    addr_mainnet = addr_mainnet[:i] + '<u><b>' + \
                        addr_mainnet[i:i+1] + '</u></b>' + addr_mainnet[i+1:]
                    text = str(addr) + '\n' + str(addr_mainnet)
    ...
    def populate_modes(self):
        ...
        self.modes.addItem(_("Summary Text PIN (requires dongle replugging)")
                           if self.txdata['confirmationType'] == 1 else _("Summary Text PIN is Disabled"))
