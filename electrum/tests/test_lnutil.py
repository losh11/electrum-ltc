...
class TestLNUtil(ElectrumTestCase):
            ...
            for insert in test["inserts"]:
                ...
                try:
                    ...
                    if insert["successful"]:
                        raise Exception(
                            "Failed ({}): error was received but it shouldn't: {}".format(test["name"], e))
                else:
    ...
    def test_commitment_tx_with_all_five_HTLCs_untrimmed_minimum_feerate(self):
        ...
        htlc[2] = make_offered_htlc(local_revocation_pubkey, remote_htlcpubkey,
                                    local_htlcpubkey, bitcoin.sha256(htlc_payment_preimage[2]))

        htlc_cltv_timeout[3] = 503
        htlc_payment_preimage[3] = b"\x03" * 32
        htlc[3] = make_offered_htlc(local_revocation_pubkey, remote_htlcpubkey,
                                    local_htlcpubkey, bitcoin.sha256(htlc_payment_preimage[3]))

        htlc_cltv_timeout[0] = 500
        htlc_payment_preimage[0] = b"\x00" * 32
        htlc[0] = make_received_htlc(local_revocation_pubkey, remote_htlcpubkey, local_htlcpubkey, bitcoin.sha256(
            htlc_payment_preimage[0]), htlc_cltv_timeout[0])

        htlc_cltv_timeout[1] = 501
        htlc_payment_preimage[1] = b"\x01" * 32
        htlc[1] = make_received_htlc(local_revocation_pubkey, remote_htlcpubkey, local_htlcpubkey, bitcoin.sha256(
            htlc_payment_preimage[1]), htlc_cltv_timeout[1])

        htlc_cltv_timeout[4] = 504
        htlc_payment_preimage[4] = b"\x04" * 32
        htlc[4] = make_received_htlc(local_revocation_pubkey, remote_htlcpubkey, local_htlcpubkey, bitcoin.sha256(
            htlc_payment_preimage[4]), htlc_cltv_timeout[4])
        ...
        for num, msat in [(0, 1000 * 1000),
                          ...
                          (3, 3000 * 1000),
                          (4, 4000 * 1000)]:
            htlc_obj[num] = UpdateAddHtlc(amount_msat=msat, payment_hash=bitcoin.sha256(
                htlc_payment_preimage[num]), cltv_expiry=0, htlc_id=None, timestamp=0)
        ...
        our_commit_tx = make_commitment(
            ...
            fees_per_participant=calc_fees_for_commitment_tx(num_htlcs=len(
                htlcs), feerate=local_feerate_per_kw, is_local_initiator=True),
            htlcs=htlcs)
        self.sign_and_insert_remote_sig(
            our_commit_tx, remote_funding_pubkey, remote_signature, local_funding_pubkey, local_funding_privkey)
        ...
        for i in range(5):
            self.assertEqual(output_htlc_tx[i][1], self.htlc_tx(htlc[i], htlc_output_index[i],
                                                                ...
                                                                output_htlc_tx[i][0], htlc_cltv_timeout[
                                                                    i] if not output_htlc_tx[i][0] else 0,
    ...
    def test_extract_commitment_number_from_tx(self):
        ...
        self.assertEqual(commitment_number, extract_ctn_from_tx(
    ...
    def test_key_derivation(self):
        ...
        self.assertEqual(base_point, bfh(
            '036d6caac248af96f6afa7f904f550253a0f3ef3f5aa2fe6838a95b216691468e2'))
        per_commitment_point = secret_to_pubkey(per_commitment_secret)
        self.assertEqual(per_commitment_point, bfh(
            '025f7117a78150fe2ef97db7cfc83bd57b2e2c0d0dd25eaf467a4a1c2a45ce1486'))
        localpubkey = derive_pubkey(base_point, per_commitment_point)
        self.assertEqual(localpubkey, bfh(
    ...
    def test_split_host_port(self):
        ...
        self.assertEqual(split_host_port(
            "[2601:602:8800:9a:dc59:a4ff:fede:24a9]:9735"), ("2601:602:8800:9a:dc59:a4ff:fede:24a9", "9735"))
        self.assertEqual(split_host_port(
            "[2601:602:8800::a4ff:fede:24a9]:9735"), ("2601:602:8800::a4ff:fede:24a9", "9735"))
        self.assertEqual(split_host_port(
            "kæn.guru:8000"), ("kæn.guru", "8000"))
        ...
        self.assertEqual(split_host_port("electrum.org:8000"),
                         ("electrum.org", "8000"))
        self.assertEqual(split_host_port("electrum.org"),
                         ("electrum.org", "9735"))
    ...
    def test_extract_nodeid(self):
        ...
        self.assertEqual(extract_nodeid(
            "00" * 33 + "@localhost"), (b"\x00" * 33, "localhost"))
    ...
    def test_ln_features_supports(self):
        ...
        self.assertFalse(f_null.supports(
            LnFeatures.OPTION_DATA_LOSS_PROTECT_OPT))
        self.assertFalse(f_null.supports(
            LnFeatures.OPTION_DATA_LOSS_PROTECT_REQ))
        self.assertTrue(f_opt.supports(
            LnFeatures.OPTION_DATA_LOSS_PROTECT_OPT))
        self.assertTrue(f_opt.supports(
            LnFeatures.OPTION_DATA_LOSS_PROTECT_REQ))
        self.assertTrue(f_req.supports(
            LnFeatures.OPTION_DATA_LOSS_PROTECT_OPT))
        self.assertTrue(f_req.supports(
            LnFeatures.OPTION_DATA_LOSS_PROTECT_REQ))
        self.assertTrue(f_optreq.supports(
            LnFeatures.OPTION_DATA_LOSS_PROTECT_OPT))
        self.assertTrue(f_optreq.supports(
            LnFeatures.OPTION_DATA_LOSS_PROTECT_REQ))
    ...
    def test_lnworker_decode_channel_update_msg(self):
        msg_without_prefix = bytes.fromhex(
            "")
        # good messages
        self.assertNotEqual(
            None,
    ...
    def test_channel_type(self):
        ...
        self.assertTrue(
            ChannelType.OPTION_STATIC_REMOTEKEY.complies_with_features(features))
        ...
        channel_type = ChannelType(
            0b10000000001000000000010).discard_unknown_and_check()
