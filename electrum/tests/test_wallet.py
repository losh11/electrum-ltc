...
class FakeFxThread:
    ...
    remove_thousands_separator = staticmethod(
        FxThread.remove_thousands_separator)
...
class TestFiat(ElectrumTestCase):
    def setUp(self):
        ...
        self.assertEqual('1,000.00', self.fx.ccy_amount_str(
            default_fiat, commas=True))

    def test_save_fiat_and_reset(self):
        self.assertEqual(False, Abstract_Wallet.set_fiat_value(
            self.wallet, txid, ccy, '1000.01', self.fx, self.value_sat))
        saved = self.fiat_value[ccy][txid]
        self.assertEqual('1,000.01', self.fx.ccy_amount_str(
            Decimal(saved), commas=True))
        self.assertEqual(True,       Abstract_Wallet.set_fiat_value(
...
class TestCreateRestoreWallet(WalletTestCase):
    ...
    def test_restore_wallet_from_text_xkey_that_is_also_a_valid_electrum_seed_by_chance(self):
        ...
        wallet = d['wallet']  # type: Standard_Wallet
        self.assertEqual(
            text, wallet.keystore.get_master_private_key(password=None))
    ...
    def test_restore_wallet_from_text_addresses(self):
        ...
        d = restore_wallet_from_text(
            text, path=self.wallet_path, config=self.config)
...
class TestWalletPassword(WalletTestCase):
    ...
    def test_update_password_with_app_restarts(self):
        ...
        asyncio.run_coroutine_threadsafe(
            wallet.stop(), self.asyncio_loop).result()
